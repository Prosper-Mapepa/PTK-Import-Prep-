...
import {
  resolveNicheColumns,
  toAppilyColumnMap,
  validateNicheTransferProspectsHeaders,
...
import {
  applyIntendedTransferDateFixes,
...
type StepId = 'upload' | 'format' | 'aoi' | 'ceeb' | 'transferDate' | 'export'
...
export default function NicheTransferProspectsPrep() {
  ...
  const [welcomeOpen, setWelcomeOpen] = useState(
    () => !isWelcomeDismissed(NICHE_TRANSFER_PROSPECTS_WELCOME_KEY),
  )
  ...
  const exportName = useMemo(() => {
    if (!fileName) return 'niche-transfer-prospects-prepared.csv'
  ...
  async function handleUpload(file: File) {
    if (!/\.csv$/i.test(file.name)) {
      setError('Please upload a Niche Transfer Prospects CSV file (.csv).')
      return
    }
    ...
    try {
      ...
      validateNicheTransferProspectsHeaders(mapped)
      ...
      const dated = applyIntendedTransferDateFixes(padded.rows, mapped)
  ...
  async function handleContinue() {
    if (step === 'upload') {
      if (!fileName) {
        setError('Upload a Niche Transfer Prospects CSV before continuing.')
        return
      }
      setError('')
      setStep('format')
    ...
    if (step === 'ceeb') {
      if (ceebBusy) return
      setStep('transferDate')
      return
    }
    if (step === 'transferDate') {
      setStep('export')
    }
  }
  ...
  return (
    <div className="app-shell">
      <WelcomeModal
        ...
        storageKey={NICHE_TRANSFER_PROSPECTS_WELCOME_KEY}
        title="Niche Transfer Prospects"
        lead="Prepare weekly Niche transfer prospect files for Slate in a few guided steps."
        steps={[
          'Download the latest file from SFTP incoming/Niche/Prospects (…_Prospects_Transfer_YYYY_MM_DD.csv).',
          'Clean names, emails, and mailing addresses; pad ZIP and CollegeCEEB leading zeros.',
          'Add CMU AOI after MajorCIP using the MajorCIP → AOI crosswalk.',
          'Fill missing CollegeCEEB codes (CEEB reference first, then College Board).',
          `Update IntendedTransferDate when blank or in the past → ${FALL_START}.`,
          'Export CSV, upload as Niche Transfer Prospects, then Remap → Value Mappings.',
        ]}
      ...
      <section className="card">
        {step === 'upload' && (
          <>
            <div className="card-header">
              <h2>Upload Niche Transfer Prospects file</h2>
              <p>
                SFTP: <code>incoming/Niche/Prospects</code> · Wednesdays · Pattern:{' '}
                <code>…_Prospects_Transfer_YYYY_MM_DD.csv</code>
              </p>
            </div>
            ...
            {fileName ? (
              ...
              >
                ...
                <p className="upload-hint">Niche Transfer Prospects · .csv only</p>
              </div>
            )}
        ...
        {step === 'format' && columns && appilyColumns && (
          <>
            <div className="card-header">
              <h2>Format scan</h2>
              <p>
                Names, emails, mailing addresses, ZIP zeros, and existing CollegeCEEB padding were
        ...
        {step === 'aoi' && columns && (
          <>
            <div className="card-header">
              <h2>CMU AOI</h2>
              <p>
                Inserts <code>CMU AOI</code> after <code>MajorCIP</code> using the MajorCIP → AOI
                crosswalk (same VLOOKUP as the ticket).
              </p>
            </div>
        ...
        {step === 'ceeb' && columns && (
          <>
            <div className="card-header">
              ...
              <p>
                Missing <code>CollegeCEEB</code> values are filled from the bundled CEEB reference
                first, then College Board online. Leading zeros are padded to 4 digits.
              </p>
            </div>
        ...
        {step === 'transferDate' && columns && (
          <>
            <div className="card-header">
              <h2>Intended transfer date</h2>
              <p>
                <code>IntendedTransferDate</code> maps to start term. Blank or past dates are
                updated to the upcoming Fall start → <strong>{dateReplacement}</strong>.
              </p>
            </div>
            <div className="stats-grid">
              <StatCard value={rows.length} label="Prospects" />
            ...
            {dateChanges.length > 0 ? (
              ...
              <p className="empty-state">No IntendedTransferDate values needed updating.</p>
            )}
          </>
        )}
        ...
        {step === 'export' && (
          <>
            <div className="card-header">
              ...
              <p>
                Slate source format: <strong>Niche Transfer Prospects</strong>. After upload:{' '}
                <strong>Remap → Value Mappings</strong>, refresh, and run a{' '}
                <strong>Retroactive Refresh</strong> if needed.
              </p>
            </div>
            <div className="stats-grid">
