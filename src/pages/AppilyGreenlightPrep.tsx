...
import { resolveColumns, validateAppilyGreenlightHeaders } from '../appily/lib/columns'
...
function isGreenlightFile(file: File) {
  return FILE_RE.test(file.name)
}

export default function AppilyGreenlightPrep() {
  ...
  const [welcomeOpen, setWelcomeOpen] = useState(() => !isWelcomeDismissed(GREENLIGHT_WELCOME_KEY))
  ...
  const exportFileName = useMemo(() => {
    if (!fileName) return 'greenlight-prepared.csv'
  ...
  async function handleUpload(file: File) {
    if (!isGreenlightFile(file)) {
      setError('Please upload a College Greenlight CSV file (.csv).')
      return
    }
    ...
    try {
      ...
      validateAppilyGreenlightHeaders(mapped)
      ...
      const issues = findAddressIssues(addressed.rows, mapped)
  ...
  function handleContinue() {
    if (step === 'upload') {
      if (!fileName || rows.length === 0) {
        setError('Upload a College Greenlight CSV before continuing.')
        return
      }
      setError('')
      setStep('format')
  ...
  return (
    <div className="app-shell">
      <WelcomeModal
        ...
        storageKey={GREENLIGHT_WELCOME_KEY}
        title="College Greenlight Inquiries"
        lead="Prepare Cappex College Greenlight inquiry files for Slate in a few guided steps."
        steps={[
        ...
        {step === 'upload' && (
          <>
            <div className="card-header">
              <h2>Upload Greenlight file</h2>
              <p>
                Source: Slate SFTP <code>/incoming/appily/inquiries</code> (weekly). Format:{' '}
                <strong>Appily – College Greenlight Inquiries (Cappex)</strong>.
              </p>
            </div>
            ...
            {fileName ? (
              ...
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label="Upload Greenlight CSV"
              >
                {loading && <Loader message="Reading file…" variant="overlay" size="sm" />}
                ...
                <p className="upload-title">Drop Greenlight CSV here, or click to browse</p>
                <p className="upload-hint">
                  Appily – College Greenlight Inquiries (Cappex) · .csv only
                </p>
              </div>
            )}
