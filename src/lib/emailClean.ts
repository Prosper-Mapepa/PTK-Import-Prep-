...
export function isEmailField(header: string): boolean {
  ...
  return (
    ...
    normalized === 'school email' ||
    /^e-?mail\b/.test(normalized)
  )
}
...
export function cleanEmails(rows: PtkRow[], headers: string[]): {
  rows: PtkRow[]
  changes: AddressCleanChange[]
} {
  const emailFields = headers.filter(isEmailField)
