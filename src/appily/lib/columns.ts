...
const ALIASES: Record<keyof ColumnMap, string[]> = {
  ...
  ceebCode: ['ceeb_code', 'ceeb', 'college_ceeb'],
...
export function resolveColumns(headers: string[]): ColumnMap {
  ...
  return {
    ...
    expectedTransferTerm: find(ALIASES.expectedTransferTerm),
    currentCollege: find(ALIASES.currentCollege),
    ceebCode: find(ALIASES.ceebCode),
    id: find(ALIASES.id),
  }
}

/** Prefer expected_transfer_term by name; fall back to column O (index 14). */
export function resolveTransferTermColumn(
  headers: string[],
  columns: ColumnMap,
): ColumnMap {
  if (columns.expectedTransferTerm) return columns
  if (headers.length >= 15) {
    return { ...columns, expectedTransferTerm: headers[14] }
  }
  return columns
}
...
export function validateAppilyFreshmanHeaders(columns: ColumnMap): void {
...
/** @deprecated Use validateAppilyFreshmanHeaders */
export function validateAppilyHeaders(columns: ColumnMap): void {
  validateAppilyFreshmanHeaders(columns)
}

export function validateAppilyTransferHeaders(columns: ColumnMap): void {
  const missing: string[] = []
  if (!columns.firstName) missing.push('first_name')
  if (!columns.lastName) missing.push('last_name')
  if (!columns.expectedTransferTerm) missing.push('expected_transfer_term')

  if (missing.length > 0) {
    throw new Error(
      `Missing required Cappex columns: ${missing.join(', ')}. Check that this is an Appily Transfer Inquiries file.`,
    )
  }
}

export function validateAppilyGreenlightHeaders(columns: ColumnMap): void {
  const missing: string[] = []
  if (!columns.firstName) missing.push('first_name')
  if (!columns.lastName) missing.push('last_name')
  if (!columns.email) missing.push('email_address')
  if (!columns.address1) missing.push('address_1')

  if (missing.length > 0) {
    throw new Error(
      `Missing required Cappex columns: ${missing.join(', ')}. Check that this is a College Greenlight Inquiries file.`,
    )
  }
}

export function validateAppilyTransferProspectsHeaders(columns: ColumnMap): void {
  const missing: string[] = []
  if (!columns.firstName) missing.push('first_name')
  if (!columns.lastName) missing.push('last_name')
  if (!columns.email) missing.push('email_address')
  if (!columns.currentCollege) missing.push('current_college_name')
  if (!columns.expectedTransferTerm) missing.push('expected_transfer_term')

  if (missing.length > 0) {
    throw new Error(
      `Missing required Cappex columns: ${missing.join(', ')}. Check that this is an Appily Transfer Prospects file.`,
    )
  }
}
